<?php namespace feedthemfacebook;
/**
...
class FTS_Facebook_Feed extends feed_them_social_functions
{
    ...
    {
        wp_enqueue_style('fts-feeds', plugins_url('feed-them-facebook/feeds/css/styles.css'));
    }
    ...
    function fts_fb_func($atts)
    {
        ...
        if(!is_plugin_active('feed-them-premium/feed-them-premium.php') && $FB_Shortcode['posts'] > '6'){
     ...
     //   	print_r($feed_data);
        ...
        if (!isset($_GET['load_more_ajaxing'])) {
            ...
            if (is_plugin_active('feed-them-premium/feed-them-premium.php')) {
                ...
                $FTS_FB_OUTPUT .= isset($FB_Shortcode['title']) && $FB_Shortcode['title'] !== 'no' ? '<h1><a href="' . $fts_view_fb_link . '" target="_blank">' . $page_data->name . '</a></h1>' : '';
                //Description
    ...
    function fts_facebook_post_photo($FBlink, $FB_Shortcode, $photo_from, $photo_source)
    {
        if ($FB_Shortcode['type'] == 'album_photos' || $FB_Shortcode['type'] == 'albums') {
            ...
            if ($FB_ShortcodePopup == 'yes') {
                $output = isset($FB_Shortcode['popup']) && $FB_Shortcode['popup'] == 'yes' ? '<div class="fts-fb-caption"><a href="' . $FBlink . '" class="fts-view-on-facebook-link" target="_blank">' . __('View on Facebook', 'feed-them-facebook') . '</a></div> ' : '';

                $output .= '<a href="' . $photo_source . '" target="_blank" class="fts-jal-fb-picture fts-fb-large-photo"><img border="0" alt="' . $photo_from . '" src="' . $photo_source . '"/></a>';
    ...
    function fts_facebook_post_see_more($FBlink, $lcs_array, $FBtype, $FBpost_id = NULL, $FB_Shortcode, $FBpost_user_id = NULL, $FBpost_single_id = NULL, $single_event_id = null, $post_data)
    {
        ...
        switch ($FBtype) {
            case 'events':
                $output = '<a href="http://facebook.com/events/' . $single_event_id . '" target="_blank" class="fts-jal-fb-see-more">' . __('View on Facebook', 'feed-them-facebook') . '</a>';
                return $output;
            case 'photo':
                if (!empty($FBlink)) {
                ...
                $output .= '&nbsp;' . __('View on Facebook', 'feed-them-facebook') . '</a>';
                return $output;
            case 'app':
            case 'cover':
            case 'profile':
            ...
            default:
                ...
                if ($FB_Shortcode['type'] == 'reviews' && is_plugin_active('feed-them-social-facebook-reviews/feed-them-social-facebook-reviews.php')) {
                    $output = '';
                    $output .= ' <a href="https://facebook.com/' . $FB_Shortcode['id'] . '/reviews" target="_blank" class="fts-jal-fb-see-more">' . __('See More Reviews', 'feed-them-facebook') . '</a>';
                } else {
                    $output = '<a href="https://facebook.com/' . $FBpost_user_id . '/posts/' . $FBpost_single_id . '" target="_blank" class="fts-jal-fb-see-more">';
                    $output .= '' . $lcs_array['likes'] . ' ' . $lcs_array['comments'] . ' &nbsp;&nbsp;&nbsp;' . __('View on Facebook', 'feed-them-facebook') . '</a>';
                }
        ...
        if (false !== ($transient_exists = $this->fts_check_feed_cache_exists($fb_cache_name)) and !isset($_GET['load_more_ajaxing'])) {
            ...
            elseif ($FB_Shortcode['type'] == 'reviews') {
                if (is_plugin_active('feed-them-social-facebook-reviews/feed-them-social-facebook-reviews.php')) {
                    ...
                    return 'Please Purchase and Activate the Feed Them Facebook Reviews plugin.';
                    exit;
                }
            } else {
                $mulit_data = array('page_data' => 'https://graph.facebook.com/' . $FB_Shortcode['id'] . '?fields=id,name,description&access_token=' . $access_token . $language . '');
                //Check If Ajax next URL needs to be used
                $mulit_data['feed_data'] = isset($_REQUEST['next_url']) ? $_REQUEST['next_url'] : 'https://graph.facebook.com/' . $FB_Shortcode['id'] . '/feed?fields=id,caption,created_time,description,from,icon,link,message,name,object_id,picture,full_picture,uplace,shares,source,status_type,story,to,type&limit=' . $FB_Shortcode['posts'] . '&access_token=' . $access_token . $language . '';
            }
    ...
    function load_popup_scripts($FB_Shortcode)
    {
        if ($FB_Shortcode['popup'] == 'yes') {
            ...
            if (isset($fts_fix_magnific) && $fts_fix_magnific !== '1') {
                wp_enqueue_style('fts-popup', plugins_url('feed-them-facebook/feeds/css/magnific-popup.css'));
            }
            wp_enqueue_script('fts-popup-js', plugins_url('feed-them-facebook/feeds/js/magnific-popup.js'));
            wp_enqueue_script('fts-images-loaded', plugins_url('feed-them-facebook/feeds/js/imagesloaded.pkgd.min.js'));
            if (!isset($FB_Shortcode['video_album']) && $FB_Shortcode['video_album'] == 'yes') {
                wp_enqueue_script('fts-global', plugins_url('feed-them-facebook/feeds/js/fts-global.js'), array('jquery'));
            }
        }
            ...
            if (!isset($_GET['load_more_ajaxing']) && !isset($_REQUEST['fts_no_more_posts']) && !empty($FB_Shortcode['loadmore'])) {
                ...
                if ($FBtype && $FB_Shortcode['type'] == 'albums' || $FBtype && $FB_Shortcode['type'] == 'album_photos' && $FB_Shortcode['video_album'] !== 'yes' || $FB_Shortcode['grid'] == 'yes') {
                    ...
                    if ($FB_Shortcode['type'] == 'reviews') {
                        $LOADMORE_OUPUT .= 'jQuery("#loadMore_' . $fts_dynamic_name . '").replaceWith(\'<div class="fts-fb-load-more no-more-posts-fts-fb">' . __('No More Reviews', 'feed-them-facebook') . '</div>\');';
                    } else {
                        $LOADMORE_OUPUT .= 'jQuery("#loadMore_' . $fts_dynamic_name . '").replaceWith(\'<div class="fts-fb-load-more no-more-posts-fts-fb">' . __('No More Photos', 'feed-them-facebook') . '</div>\');';
                    ...
                    if ($FB_Shortcode['type'] == 'reviews') {
                        ...
                        $LOADMORE_OUPUT .= 'jQuery("#loadMore_' . $fts_dynamic_name . '").replaceWith(\'<div class="fts-fb-load-more no-more-posts-fts-fb">' . __('No More Posts', 'feed-them-facebook') . '</div>\');';
                    }
                    $LOADMORE_OUPUT .= 'jQuery("#loadMore_' . $fts_dynamic_name . '").removeAttr("id");';
                    $LOADMORE_OUPUT .= 'jQuery(".' . $fts_dynamic_class_name . '").unbind("scroll");';
                    $LOADMORE_OUPUT .= '}';
                }
                $LOADMORE_OUPUT .= 'jQuery("#loadMore_' . $fts_dynamic_name . '").html("' . __('Load More', 'feed-them-facebook') . '");';
