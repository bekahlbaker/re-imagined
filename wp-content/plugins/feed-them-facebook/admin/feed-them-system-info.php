...
?>
		...
		<?php _e( 'Get Extensions Here!', 'feed-them-facebook' ); ?>
		</a>
		<h2>
		<?php _e( 'System Info', 'feed-them-facebook' ); ?>
		</h2>
		<p>
		<?php _e( 'Please click the box below and copy the report. You will need to paste this information along with your question in our', 'feed-them-facebook' ); ?>
		<a href="http://www.slickremix.com/support-forum/" target="_blank">
		<?php _e( 'Support Forum', 'feed-them-facebook' ); ?>
		</a>.
		<?php _e( 'Ask your question then paste the copied text below it.  To copy the system info, click below then press Ctrl + C (PC) or Cmd + C (Mac).', 'feed-them-facebook' ); ?>
		</p>
		<form action="<?php echo esc_url( admin_url( 'admin.php?page=fts-system-info-submenu-page' ) ); ?>" method="post" dir="ltr" >
		<textarea readonly="readonly" onclick="this.focus();this.select()" id="system-info-textarea" name="fts-sysinfo" title="<?php _e( 'To copy the system info, click here then press Ctrl + C (PC) or Cmd + C (Mac).', 'feed-them-facebook' ); ?>">
### Begin System Info ###
		<?php
...
Feed Them Facebook Version: <?php echo FEED_THEM_FACEBOOK_VERSION. "\n"; ?>
