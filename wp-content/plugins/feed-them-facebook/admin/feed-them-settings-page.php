...
class FTS_settings_page
{
    ...
    function feed_them_settings_page()
    {
        ...
        if (!function_exists('curl_init')) {
            print '<div class="error"><p>' . __('Warning: cURL is not installed on this server. It is required to use this plugin. Please contact your host provider to install this.', 'feed-them-facebook') . '</p></div>';
        } ?>

        <div class="feed-them-social-admin-wrap">
            <div class="fts-backg"></div>
            <div class="fts-content">
                <h1><?php _e('Feed Them Facebook', 'feed-them-facebook'); ?></h1>
                <div class="use-of-plugin"><?php _e('Please select what type of feed you would like to see. Then you can copy and paste the shortcode to a page, post or widget.', 'feed-them-facebook'); ?></div>
                <div class="feed-them-icon-wrap">
                    <?php
                    //show the js for the discount option under social icons on the settings page
                    if (!is_plugin_active('feed-them-premium/feed-them-premium.php')) { ?>
                        <div id="discount-for-review"><?php _e('15% off Premium Version', 'feed-them-facebook'); ?></div>
                        <div class="discount-review-text">
                            <a href="http://www.slickremix.com/downloads/feed-them-social-premium-extension/" target="_blank"><?php _e('Share here', 'feed-them-facebook'); ?></a> <?php _e('and receive 15% OFF your total order.', 'feed-them-facebook'); ?>
                        </div>
                    <?php } ?>
                </div>
                <form class="feed-them-social-admin-form">
                    <select id="shortcode-form-selector">
                        <option value=""><?php _e('Select a social network to get started', 'feed-them-facebook'); ?> </option>
                        <option value="fb-page-shortcode-form"><?php _e('Facebook Feed', 'feed-them-facebook'); ?></option>
                    </select>
                ...
                <div class="feed-them-clear-cache">
                    <h2><?php _e('Clear All Cache Options', 'feed-them-facebook'); ?></h2>
                    <div class="use-of-plugin"><?php _e('Please Clear Cache if you have changed a Feed Them Facebook Shortcode. This will Allow you to see the changes right away.', 'feed-them-facebook'); ?></div>
                    ...
                    <form method="post" action="?page=feed-them-settings-page&cache=clearcache">
                        <input class="feed-them-social-admin-submit-btn" type="submit" value="<?php _e('Clear All FTS Feeds Cache', 'feed-them-facebook'); ?>"/>
                    </form>
                ...
                <form method="post" class="fts-color-settings-admin-form" action="options.php">
                    <p>
                        ...
                        if (get_option('fts_clear_cache_developer_mode') == '1') { ?>
                            <?php _e('Cache will clear on every page load now', 'feed-them-facebook'); ?><?php
                        } else { ?>
                            <?php _e('Developer Mode: Clear cache on every page load', 'feed-them-facebook'); ?><?php
                        }
                        ?>
                    </p>
                    <select id="fts_admin_bar_menu" name="fts_admin_bar_menu">
                        <option value="show-admin-bar-menu" <?php if ($ftsAdminBarMenu == 'show-admin-bar-menu') echo 'selected="selected"'; ?>><?php _e('Show Admin Bar Menu', 'feed-them-facebook'); ?></option>
                        <option value="hide-admin-bar-menu" <?php if ($ftsAdminBarMenu == 'hide-admin-bar-menu') echo 'selected="selected"'; ?>><?php _e('Hide Admin Bar Menu', 'feed-them-facebook'); ?></option>
                    </select>
                    <div class="feed-them-custom-css">
                        ...
                        <div style="float:left; max-width:400px; margin-right:30px;">
                            <h2><?php _e('FaceBook & Twitter Date Format', 'feed-them-facebook'); ?></h2>
                            ...
                            <fieldset>
                                <select id="fts-date-and-time-format" name="fts-date-and-time-format">
                                    ...
                                    <option value="one-day-ago" <?php if ($ftsDateTimeFormat == 'one-day-ago') echo 'selected="selected"'; ?>><?php _e('1 day ago', 'feed-them-facebook'); ?></option>
                                    <option value="fts-custom-date" <?php if ($ftsDateTimeFormat == 'fts-custom-date') echo 'selected="selected"'; ?>><?php _e('Use Custom Date and Time Option Below', 'feed-them-facebook'); ?></option>
                                </select>
                            </fieldset>
                            ...
                            <div class="custom_time_ago_wrap" style="display:none;">
                                <h2><?php _e('Translate words for 1 day ago option.', 'feed-them-facebook'); ?></h2>
                            ...
                            <h2 style="border-top:0px; margin-bottom:4px !important;"><?php _e('Custom Date and Time', 'feed-them-facebook'); ?></h2>
                            ...
                            <p style="margin:12px 0 !important;">
                                <input name="fts-custom-date" style="max-width:105px;" class="fts-color-settings-admin-input" id="fts-custom-date" placeholder="<?php _e('Date', 'feed-them-facebook'); ?>" value="<?php echo get_option('fts-custom-date'); ?>"/>
                                <input name="fts-custom-time" style="max-width:75px;" class="fts-color-settings-admin-input" id="fts-custom-time" placeholder="<?php _e('Time', 'feed-them-facebook'); ?>" value="<?php echo get_option('fts-custom-time'); ?>"/>
                            </p>
                            <div><?php _e('This will override the date and time format above.', 'feed-them-facebook'); ?>
                                <br/><a href="https://codex.wordpress.org/Formatting_Date_and_Time" target="_blank"><?php _e('Options for custom date and time formatting.', 'feed-them-facebook'); ?></a>
                            </div>
                        </div>
                        <div style="float:left; max-width:330px;"><h2><?php _e('TimeZone', 'feed-them-facebook'); ?></h2>
                            <fieldset>
                        ...
                        <div class="clear"></div>
                        <br/>
                        <h2><?php _e('Custom CSS Option', 'feed-them-facebook'); ?></h2>
                        <p>
                            ...
                            if (get_option('fts-color-options-settings-custom-css') == '1') { ?>
                                <strong><?php _e('Checked:', 'feed-them-facebook'); ?></strong> <?php _e('Custom CSS option is being used now.', 'feed-them-facebook'); ?><?php
                            } else { ?>
                                <strong><?php _e('Not Checked:', 'feed-them-facebook'); ?></strong> <?php _e('You are using the default CSS.', 'feed-them-facebook'); ?><?php
                            }
                            ?>
                        </p>
                        <label class="toggle-custom-textarea-show"><span><?php _e('Show', 'feed-them-facebook'); ?></span><span class="toggle-custom-textarea-hide"><?php _e('Hide', 'feed-them-facebook'); ?></span> <?php _e('custom CSS', 'feed-them-facebook'); ?>
                        </label>
                        <div class="clear"></div>
                        <div class="fts-custom-css-text"><?php _e('Thanks for using our plugin :) Add your custom CSS additions or overrides below.', 'feed-them-facebook'); ?></div>
                    ...
                    <div class="feed-them-custom-logo-css">
                        <h2><?php _e('Disable Magnific Popup CSS', 'feed-them-facebook'); ?></h2>
                        <p>
                            <input name="fts_fix_magnific" class="fts-powered-by-settings-admin-input" type="checkbox" id="fts_fix_magnific" value="1" <?php echo checked('1', get_option('fts_fix_magnific')); ?>/> <?php _e('Check this if you are experiencing problems with your theme(s) or other plugin(s) popups.', 'feed-them-facebook'); ?>
                        </p>
                        <br/>

                        <?php if (is_plugin_active('feed-them-premium/feed-them-premium.php')) { ?>
                            <h2><?php _e('Fix Load More Error', 'feed-them-facebook'); ?></h2>
                            <p>
                                <input name="fts_fix_loadmore" class="fts-powered-by-settings-admin-input" type="checkbox" id="fts_fix_loadmore" value="1" <?php echo checked('1', get_option('fts_fix_loadmore')); ?>/> <?php _e('Check this if you are using the loadmore button for Facebook or Instagram and are seeing a bunch of code under it.', 'feed-them-facebook'); ?>
                            </p>
                            <br/>
                        <?php } ?>

                        <h2><?php _e('Fix Internal Server Error', 'feed-them-facebook'); ?></h2>
                        <p>
                            <input name="fts_curl_option" class="fts-powered-by-settings-admin-input" type="checkbox" id="fts_curl_option" value="1" <?php echo checked('1', get_option('fts_curl_option')); ?>/> <?php _e('Check this option if you are getting a 500 Internal Server Error when trying to load a page with our feed on it.', 'feed-them-facebook'); ?>
                        </p>
                        <br/>

                        <h2><?php _e('Powered by Text', 'feed-them-facebook'); ?></h2>
                        <p>
                            ...
                            if (get_option('fts-powered-text-options-settings') == '1') { ?>
                                <strong><?php _e('Checked:', 'feed-them-facebook'); ?></strong> <?php _e('You are not showing the Powered by Logo.', 'feed-them-facebook'); ?><?php
                            } else { ?>
                                <strong><?php _e('Not Checked:', 'feed-them-facebook'); ?></strong><?php _e('The Powered by text will appear in the site. Awesome! Thanks so much for sharing.', 'feed-them-facebook'); ?><?php
                            }
                            ?>
                        </p>
                        <br/>
                        <input type="submit" class="feed-them-social-admin-submit-btn" value="<?php _e('Save All Changes', 'feed-them-facebook') ?>"/>
                        <div class="clear"></div>
        ...
        <h1 class="plugin-author-note"><?php _e('Plugin Authors Note', 'feed-them-facebook'); ?></h1>
        <div class="fts-plugin-reviews">
            <div class="fts-plugin-reviews-rate"><?php _e(' Feed Them Facebook was created by 2 Brothers, Spencer and Justin Labadie. That’s it, 2 people! We spend all our time creating and supporting this plugin. Show us some love if you like our plugin and leave a quick review for us, it will make our day!', 'feed-them-facebook'); ?>
                <a href="https://wordpress.org/support/view/plugin-reviews/feed-them-facebook" target="_blank"><?php _e('Leave us a Review', 'feed-them-facebook'); ?>
                    ★★★★★</a>
            </div>
            <div class="fts-plugin-reviews-support"><?php _e('If you\'re having troubles getting setup please contact us. We will respond within 24hrs, but usually within 1-6hrs.', 'feed-them-facebook'); ?>
                <a href="http://www.slickremix.com/support-forum/forum/feed-them-social-2" target="_blank"><?php _e('Support Forum', 'feed-them-facebook'); ?></a>
        ...
        <script>
            jQuery(function () {
                ...
                jQuery('#facebook-messages-selector').change(function () {
                    ...
                    if (facebooktype == 'album_videos') {
                        ...
                        jQuery(".feed-them-social-admin-input-label:contains('Album')").html("<?php _e('Video Album ID (required)', 'feed-them-facebook') ?>");
                        jQuery(".feed-them-social-admin-input-label:contains('# of Posts')").html("<?php _e('# of Videos', 'feed-them-facebook') ?>");
                    }
                    else {
                        ...
                        jQuery(".feed-them-social-admin-input-label:contains('Video Album ID (required)')").html("<?php _e('Album ID', 'feed-them-facebook') ?><br/><small><?php _e('Leave Blank to show all photos.', 'feed-them-facebook') ?></small>");
                        jQuery(".feed-them-social-admin-input-label:contains('# of Videos')").html("<?php _e('# of Posts', 'feed-them-facebook') ?>");
                    }
                    <?php    }
                    else { ?>
                    if (facebooktype == 'album_videos') {
                    ...
                    if (facebooktype == 'albums' || facebooktype == 'album_photos' || facebooktype == 'album_videos') {
                        ...
                        jQuery('.fixed_height_option').hide();
                        jQuery(".feed-them-social-admin-input-label:contains('<?php _e('Display Posts in Grid', 'feed-them-facebook'); ?>')").parent('div').hide();
                    }
                    else {
                        jQuery('.fts-super-facebook-options-wrap').hide();
                        jQuery('.fixed_height_option').show();
                        jQuery('.fb-posts-in-grid-option-wrap').show();
                        jQuery(".feed-them-social-admin-input-label:contains('<?php _e('Display Posts in Grid', 'feed-them-facebook'); ?>')").parent('div').show();
                ...
                jQuery('#fb-grid-option').bind('change', function (e) {
                    if (jQuery('#fb-grid-option').val() == 'yes') {
                        ...
                        jQuery(".feed-them-social-admin-input-label:contains('<?php _e('Center Facebook Container?', 'feed-them-facebook'); ?>')").parent('div').show();
                    }
                    else {
                ...
                jQuery(".feed-them-social-admin-input-label:contains('<?php _e('# of Pins', 'feed-them-facebook'); ?>')").parent('div').hide();
                jQuery('#pinterest-messages-selector').bind('change', function (e) {
                    if (jQuery('#pinterest-messages-selector').val() == 'boards_list') {
                        ...
                        jQuery(".feed-them-social-admin-input-label:contains('<?php _e('# of Boards', 'feed-them-facebook'); ?>')").parent('div').show();
                        jQuery(".feed-them-social-admin-input-label:contains('<?php _e('# of Pins', 'feed-them-facebook'); ?>')").parent('div').hide();
                    }
                });
                // Pinterest options
                jQuery('#pinterest-messages-selector').bind('change', function (e) {
                    if (jQuery('#pinterest-messages-selector').val() == 'single_board_pins') {
                        ...
                        jQuery(".feed-them-social-admin-input-label:contains('<?php _e('# of Boards', 'feed-them-facebook'); ?>')").parent('div').hide();
                        jQuery(".feed-them-social-admin-input-label:contains('<?php _e('# of Pins', 'feed-them-facebook'); ?>')").parent('div').show();
                    }
                })
                // Pinterest options
                jQuery('#pinterest-messages-selector').bind('change', function (e) {
            ...
            function converter_instagram_username() {
                ...
                if (convert_instagram_username != "") {
                    ...
                    <?php $fts_instagram_tokens_array = array('da06fb6699f1497bb0d5d4234a50da75', 'da06fb6699f1497bb0d5d4234a50da75');
